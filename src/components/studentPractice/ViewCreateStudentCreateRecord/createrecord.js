...
const CreateRecord = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/student').then(response => {
            setGetTeacherData(response.data)
        }).catch(error => {
    ...
    const AddStudentDataHandler = (e) => {
        ...
        let payload = {
            ...
            parentId: Number(addStudentRecord?.parentId),
        }
        console.log("payload", payload)
        axios.post("http://localhost:8080/parent", payload).then(response => {
            Nevigate("/view-practice-student")
        }).catch(error => {
            console.log("error", error)
        })
    }
                ...
                <div className=" edit_content_main_div ">
                    ...
                    <div className="form_main_div">
                        <form onSubmit={(e) => AddStudentDataHandler(e)} className="main_form">
                            ...
                            {/* name_input_div*/}
                            <div className="name_input_div">
                                <label className="name_lable">select teacher</label><br />
                                <select onChange={(e) => {
                                    let duplicate = { ...addStudentRecord }
                                    duplicate.parentId = e.target.value
                                    setAddStudentRecord(duplicate)
                                }} required >
                                    <option value={""}>select teacher</option>
                                    {getTeacherData?.map((single) => (
                                        <option value={single.parentId}>{single.parentFullName}</option>
                                    ))}
                                </select>
                            </div>
                            {/* button_div */}
